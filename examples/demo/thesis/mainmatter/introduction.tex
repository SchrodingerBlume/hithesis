...
前。\hitindex*{zh=乔峰, en=Qiao Feng}\hitindex*{Xu Zhu}
中文按各词汉语拼音第一个字母排序，英文按该词第一个英文字母排序。

\section{术语排版举例}[Glossaries and index]

术语的定义和使用可以结合索引，灵活使用。
例如，\abbr{TSSBP}是一种应用于狄利克雷过程抽样的算法。
下次出现将是另一种格式：\abbr{TSSBP}。
还可以切换单复数例如：\abbrs{SCNA}，下次出现为：\abbrs{SCNA}。
此处体现了\LaTeX{}格式内容分离的优势。

\section{引用}[Cite]

\hitindex*{段誉}引文标注遵照GB/T7714-2005，采用顺序编码制。正文中引用文献的标示应置于所引内容最后一个字的右上角，所引文献编号用阿拉伯数字置于方括号“[ ]”中，用小4号字体的上角标。要求：

（1）引用单篇文献时，如“二次铣削\cite{cnproceed}”。

（2）同一处引用多篇文献时，各篇文献的序号在方括号内全部列出，各序号间用“，”，如
遇连续序号，可标注讫序号。如，…形成了多种数学模型\cite{cnarticle,cnproceed}…
注意此处添加\cs{inlinecite}中文空格\inlinecite{cnarticle,cnproceed}，分隔符由 gbt7714 决定。
...
\caption{打高尔夫球球的人（博士论文双语题注）}[The person playing golf (Doctoral thesis)]
\end{figure}
...
\centering
\includegraphics[width=\textwidth]{golfer}
\caption{打高尔夫球的人。注意，这里默认居中}[The person playing golf. Please note that, it is vertically center aligned by default.]
\end{minipage}
\end{figure}
...
\centering
\includegraphics[width=\textwidth]{golfer}
\caption{打高尔夫球的人。注意，此图是顶部对齐}[The person playing golf. Please note that, it is vertically top aligned.]
\end{minipage}
\end{figure}
...
\caption{打高尔夫球的人。注意，此图对齐方式是图片底部对齐}[The person playing golf. Please note that, it is vertically bottom aligned for figure.]
\end{minipage}
\centering
\begin{minipage}[t]{0.4\textwidth}
\centering
...
\centering
\subfigure[\bisubcaption{打高尔夫球的人~1}{The person playing golf}]{\label{golfer41}\includegraphics[width=0.4\textwidth]{golfer}}
\hspace{2em}
\subfigure[\bisubcaption{打高尔夫球的人~2}{The person playing golf}]{\label{golfer42}\includegraphics[width=0.4\textwidth]{golfer}}
\end{minipage}
\centering
\begin{minipage}{\textwidth}
\centering
\subfigure[\bisubcaption{打高尔夫球的人~3}{The person playing golf}]{\label{golfer43}\includegraphics[width=0.4\textwidth]{golfer}}
\hspace{2em}
\subfigure[\bisubcaption{打高尔夫球的人~4。注意，规范中没有明确规定要悬挂缩进、最后一行居中。}{The person playing golf. Here, 'hang indent' and 'center last line' are not stipulated in the regulation.}]{\label{golfer44}\includegraphics[width=0.4\textwidth]{golfer}}
\end{minipage}
\caption{打高尔夫球的人}[The person playing golf]
\end{figure}
...
\begin{figure}[t]
  ...
  \begin{minipage}{.7\linewidth}
    ...
    \begin{minipage}{\textwidth}
      \centering
      \subfigure[\bisubcaption{打高尔夫球的人~1}{The person playing golf}]{\label{golfer45}\includegraphics[width=0.4\textwidth]{golfer}}
      \hspace{4em}
      \subfigure[\bisubcaption{打高尔夫球的人~2}{The person playing golf}]{\label{golfer46}\includegraphics[width=0.4\textwidth]{golfer}}
    \end{minipage}
    \figurenote{注意：这里是中文图注添加位置（我工要求，图注在图题之上）。}
\caption{打高尔夫球的人。注意，此处我工有另外一处要求，子图图题可以位于主图题之下。但由于没有明确说明位于下方具体是什么格式，所以这里不给出举例。}[The person playing golf. Please note that, although it is appropriate to put subfigures' captions under this caption as stipulated in regulation, but its format is not clearly stated.]
  \end{minipage}
\end{figure}
...
\begin{figure}[!ht]
	...
	\begin{sideways}
		\begin{minipage}{\textheight}
			...
			\fbox{\includegraphics[width=0.2\textwidth]{golfer}}
			\fbox{\includegraphics[width=0.2\textwidth]{golfer}}
\caption{打高尔夫球的人（非规范要求）}[The person playing golf (Not stated in the regulation)]
		\end{minipage}
	\end{sideways}
\end{figure}
...
\begin{figure}[thb!]
  \centering
	...
	\draw (9, -1.5) node[ttcell]{5};
      \end{tikzpicture}
  \figurenote{图中，带有箭头的淡蓝色箭头表示肿瘤子种群的进化方向。一般地，从肿瘤组织中取用于进行二代测序的样本中含有一定程度的正常细胞污染，因此肿瘤的样本中含有正常细胞和肿瘤细胞。每一个子种群的基因组的模拟过程是把生殖细胞变异和体细胞变异加入到参考基因组中。}
\caption{肿瘤组织中各个子种群的进化示意图}[The diagram of tumor subpopulation evolution process]\label{tumor}
\end{figure}
...
表格应具有三线表格式，推荐调用~tabularray~宏包并启用它的~booktabs~库（本示例
的~hithesis.sty~里已经写好这两行）。三线的线宽、单元格边距、表内字号行距由模板
统一给定，与~v3.1e~的~tabular~老写法共用一套值，两种写法排出来版面一致；新写法
把列格式集中在~colspec~一处，标准格式如表~\ref{table1}~所示。
\begin{table}[htbp]
\caption{符合研究生院绘图规范的表格}[Table in agreement of the standard from graduate school]\label{table1}
\centering
\begin{booktabs}{colspec={ccccc}}
\toprule
...
排功能，需要长表格环境。推荐用~tabularray~的\verb|longtblr|环境：外参数
\verb|caption={中文表题}|，博士学位论文的英文表题写在~\verb|caption-en|~里，与
浮动表格的双语题注同一条链、同一个开关；续页重复表头（\verb|rowhead=1|）与续页
右上角的“表~x-x（续表）”都由模板默认给出，无需手写，所绘制的长表格的格式如
表~\ref{table2}~所示。老写法~longtable~宏包的\verb|longtable|环境仍然可用，
双语题注写\verb|\caption{中文题}[英文题]|，续表头需自己写~\verb|\endfirsthead|
那一套。

\begin{longtblr}[
  caption={中国省级行政单位一览},
  caption-en={Overview of the provincial administrative unit of China},
  label={table2}]{colspec={ccc}}
...
首先给出这种情况下的一个例子如表~\ref{table3}~所示。新写法在~colspec~里把过长的
列标为~X、再给出表格总宽度即可，如
\verb|\begin{booktabs}{width=0.7\textwidth, colspec={llX}}|；下面保留~tabularx~
的写法作为对照。
\begin{table}[htbp]
  \centering
\caption{最小的三个正整数的英文表示法}[The English construction of the smallest three positive integral numbers]\label{table3}
...
\verb|tabularx|环境生成的对公式中物理量符号进行注释的公式如式\eqref{eq:1}所示。
\begin{equation}\label{eq:1}
\ddot{\boldsymbol{\rho}}-\frac{\mu}{R_{t}^{3}}\left(3\mathbf{R_{t}}\frac{\mathbf{R_{t}\rho}}{R_{t}^{2}}-\boldsymbol{\rho}\right)=\mathbf{a}
\end{equation}
\begin{tabularx}{\textwidth}{@{}l@{\hspace{\ccwd}}r@{——}X@{}}
...
% 见 regu.tex 里同一张表的说明：盒子底与 \prevdepth 都要按正文行的深度改。
\end{tabularx}\par
\settodepth{\dimen0}{中}\kern\dimexpr\dimen0-\dp\strutbox\relax\prevdepth\dimen0
\noindent 由此方法生成的注释内容应紧邻待注释公式并置于其下方，因此不能将代码放入
\verb|table|浮动环境中。但此方法不能实现自动转页接排，可能会在当前页剩余空间不够
时，全部移动到下一页而导致当前页出现很大空白。因此在需要转页处理时，还请您手动将
需要转页的代码放入一个新的\verb|tabularx|环境中，将原来的一个\verb|tabularx|环境
拆分为两个\verb|tabularx|环境。

上面这套写法模板已经包成了\verb|eqdenote|环境，每条注释写一个\verb|\item|
即可，引导词、列距、破折号与表后的行距都由环境处理，不必自己写
\verb|tabularx|。该环境底下用的是\verb|xltabular|，放不下会自动转页接排，
不必手动拆表。若要手动拆，把后半段另起一个\verb|eqdenote|环境、可选参数传空
即可，例如\verb|\begin{eqdenote}[]|，这样后半段不再重复引导词而左缘照旧对齐。
...
\begin{sideways}
\begin{minipage}{\textheight}
\caption{不在规范中规定的横版表格}[A table style which is not stated in the regulation]
\centering
\begin{booktabs}{colspec={ccccc}}
\toprule
$D$(in) & $P_u$(lbs) & $u_u$(in) & $\beta$ & $G_f$(psi.in)\\
...
此处不给出示例，因为没法给，在
\url{https://github.com/dustincys/PlutoThesis}
...
\url{https://github.com/hithesis/hithesis}
或者oschia：
\url{https://git.oschina.net/dustincys/hithesis}
处下载。
...
对于排版有极致追求的小伙伴，可以参考
\url{http://www.texample.net/tikz/resources/}
...
\caption{捐助，注意此处是子图只用汉语图题的形式，我工规定可以不用
英语图题}[Donation, please note that it is OK to use Chinese caption
only]\label{zfb}
\end{figure}
